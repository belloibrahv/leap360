...
const TeacherResultsPage: FC<TeacherResultsPageProps> = ({ params }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        //   // For now, use the first school (in real implementation, get school from class)
        //   setSchoolName(response.data[0].name);
        // }

        const mockSchools = getMockSchools();
        if (mockSchools.length > 0) {
          setSchoolName(mockSchools[0].name);
          setSchoolId(mockSchools[0].id);
        }
        
        setClassName("SS1");
      } catch (error) {
        console.error('Failed to fetch school/class data:', error);
      }
    };
  ...
  const handleTermChange = (newTerm: string) => {
    ...
    const match = studentAllResults.find((res) => res.term === newTerm);
    if (match) {
      ...
      <div className="min-h-screen flex flex-col bg-gray-50">
        ...
        <aside className="hidden lg:flex lg:w-80 bg-white border-r border-gray-200 shadow-sm flex-col">
          <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-red-50 to-red-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-semibold text-gray-800">Class Students</h2>
        ...
        <section className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="bg-white border-b border-gray-200 px-4 sm:px-6 py-4 shadow-sm">
            <Breadcrumb items={generateTeacherResultsBreadcrumb(schoolName, schoolId, className, classId)} />
          </div>
          ...
          <div className="flex-1 p-4 sm:p-6 overflow-y-auto">
            {!selectedStudent ? (
              ...
              <div className="max-w-6xl mx-auto space-y-4 sm:space-y-6">
                  ...
                  <div className="flex items-center gap-3 sm:gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 mt-1">
                        ...
                        <span className="text-sm text-gray-600">
                          <span className="font-medium">Student ID:</span> {selectedStudent.id.slice(0, 8)}...
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                {/* Term Selection */}
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
                  <h3 className="text-lg font-semibold text-gray-800 mb-4">Select Academic Term</h3>
                  <div className="max-w-md">
                    <select
                      ...
                      value={selectedTerm}
                      onChange={(e: React.ChangeEvent<HTMLSelectElement>) => handleTermChange(e.target.value)}
                    >
